...
<head>
    ...
    <style type="text/css">
        #login-container {
            background-image: url('<?= base_url('bg-login.jpg'); ?>'); /* Gunakan base_url() untuk URL gambar */
    ...
    <script>
        $(document).ready(function() {
            $('#loginButton').on('click', function(e) {
                ...
                $.ajax({
                    url: '<?= base_url('User/login'); ?>', // Menggunakan base_url() untuk URL base
                    type: 'POST',
                    data: {
                        username: username,
                        password: password,
                    ...
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                ...
                                window.location.href = '<?= base_url(); ?>'; // Ganti dengan halaman utama Anda
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
